...
RightUpWolf[] rightupwolf = new RightUpWolf[wolfSize];
...
class RightUpWolf {
  ...
  RightUpWolf(int arr, int n, int ex, int x, int y, int a, float b, boolean at, int st, int imgsi, int ha) {
    arrI = arr;
    num = n;
    exist = ex;
    px = x;
  ...
  void far_approach_arrived() {
    houseArrived=far;
    ///////////////////////////////////////////////////house[2]'s fire
    if(px>house[2].px -disM && step==rightA_up){
    ...
    if(px>house[2].px -disS && px<house[2].px && step==rightA_up){
      if(house[2].attackingWolfNum == num){
    ...
    ///////////////////////////////////////////////////house[1]'s fire
    if(px<house[1].px +disM && step==rightDownB_up){
    ...
    if(px<house[1].px +disS && step==rightDownB_up){
      if(house[1].attackingWolfNum == num){
        houseArrived=arrived;
      }   
    }    
    ///////////////////////////////////////////////////house[0]'s fire
  if(px>house[0].px -disM && (step ==rightC_up || step==rightDownC_up)){
  ...
  if(px>house[0].px -disS && (step == rightC_up || step==rightDownC_up)){
    if(house[0].attackingWolfNum == num){
        houseArrived=arrived;
      }   
  }
  ...
  void moving() {
    if( (house[0].exist==death && house[1].exist==death && house[2].exist==death) || exist==death){
      ...
      if (px<=house[2].px) {
        ...
        if(house[1].exist==alive){
          step=rightDownB_up;
        }else if(house[0].exist==alive && house[1].exist==death){
          step=rightDownC_up;
        }else {
          step=rightSTOP;
        }
      }
    }else if(step == rightDownB_up){
      if(py<house[1].py){
        py+=2;
      }else if(py>=house[1].py){
        house[1].exist=death;
    ...
    }else if(step == rightC_up){
      if(px<house[0].px){
        px+=1;
      }else if(px>=house[0].px){
        house[0].exist=death;
      }
    }else if(step == rightDownC_up){
      if(px<house[0].px){
        px+=1;
      }else if(px>=house[0].px){
        house[0].exist = death;
